kerk.tracer = function(a){
    kerk.add(this);
    
    this.tracer       = LoadObj.tracers[a.id];
        ...
        if(this.exists && this.lastPosition){
            ...
            if(distance >= this.tracer.density){
                ...
                var tracer = new PIXI.Sprite.fromImage(this.tracer.image);
                    ...
                    tracer.rotation = this.tracer.stretch ? ToAngleObject(this.lastPosition,this.position): tracer.userData.tween_rotation.lerp(0);
            ...
            for(var i in this.tracers){
                var trac = this.tracers[i];
                    trac.alpha = trac.userData.tween_opacity.addDelta(delta);
                    
                    if(!trac.userData.stretch){
                        var scale = trac.userData.tween_scale.addDelta(delta);
